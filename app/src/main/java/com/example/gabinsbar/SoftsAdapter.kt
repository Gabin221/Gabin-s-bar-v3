...
class SoftsAdapter(private val elements: List<Element>, private val onItemClick: (Element) -> Unit) :
    RecyclerView.Adapter<SoftsAdapter.SoftsViewHolder>() {

    class SoftsViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SoftsViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_element, parent, false)
        return SoftsViewHolder(view)
    }

    override fun onBindViewHolder(holder: SoftsViewHolder, position: Int) {
